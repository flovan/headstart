...
exports = module.exports = {
	...
	logHeader: function (pkg) {
		console.log(
			...
			deps.chalk.cyan.inverse('➳  http://headstart.io') +
			'                 ' +
			deps.chalk.yellow.inverse('v' + pkg.version) + '\n'
		);
	},
	...
	logTasks: function () {
		console.log(
			deps.chalk.grey.underline('To start a new project, run:\n\n') +
			deps.chalk.magenta('headstart init [flags]') +
			deps.chalk.grey(' or ') +
			deps.chalk.magenta('hs init [flags]\n\n') +
			deps.chalk.white('--base <source>') +
			deps.chalk.grey('\t\tUse a custom boilerplate repo, eg. user/repo#branch\n')
		);
		console.log(
			deps.chalk.grey.underline('To build the project, run:\n\n') +
			deps.chalk.magenta('headstart build [flags]') +
			deps.chalk.grey(' or ') +
			deps.chalk.magenta('hs build [flags]\n\n') +
			deps.chalk.white('--s, --serve') +
			deps.chalk.grey('\t\tServe the files on a static address\n') +
			deps.chalk.white('--o, --open') +
			deps.chalk.grey('\t\tOpen up a browser for you (default Google Chrome)\n') +
			deps.chalk.white('--e, --edit') +
			deps.chalk.grey('\t\tOpen the files in your editor (default Sublime Text)\n') +
			deps.chalk.white('--p, --production') +
			deps.chalk.grey('\tMake a production ready build\n') +
			deps.chalk.white('--t, --tunnel') +
			deps.chalk.grey('\t\tTunnel your served files to the web (requires --serve)\n') +
			deps.chalk.white('--psi') +
			deps.chalk.grey('\t\t\tRun PageSpeed Insights (requires --serve and --tunnel)\n') +
			//deps.chalk.white('--key <key>') +
			//deps.chalk.grey('\t\tOptional, an API key for PSI\n') +
			deps.chalk.white('--strategy <type>') +
			deps.chalk.grey('\tRun PSI in either "desktop" (default) or "mobile" mode\n\n') +
			deps.chalk.white('--verbose') +
			deps.chalk.grey('\t\tOutput extra information while building\n')
		);
		console.log(
			deps.chalk.grey.underline('For information, run:\n\n') +
			deps.chalk.magenta('headstart [flags]') +
			deps.chalk.grey(' or ') +
			deps.chalk.magenta('hs [flags]\n\n') +
			deps.chalk.white('--i, --info,\n--h, --help') +
			deps.chalk.grey('\t\tPrint out this message\n') +
			deps.chalk.white('--v, --version') +
