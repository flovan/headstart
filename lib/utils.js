...
function _generateInject (isScript, filePath) {
	var absAssetPath = deps.path.join(settings.cwd, settings.config.dist.root);
	filePath = filePath.split(absAssetPath).pop();
	if (isScript) {
		return `<script src="${settings.config.dev.assetPrefix + filePath}"></script>`;
	}
	return `<link rel="stylesheet" href="${settings.config.dev.assetPrefix + filePath}">`;
}
