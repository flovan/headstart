...
deps.taker.task('scripts-common', function (cb) {
	return deps.vfs.src([
		...
		.on('data', function (chunk) {
			chunk.analysis = {
				isCommon: chunk.path.indexOf('common') > -1,
				isView: chunk.path.indexOf('view') > -1,
				viewName: chunk.path.split('view').pop().split('/').shift(),
				isIE: chunk.path.indexOf('ie') > -1,
				isHead: chunk.path.indexOf('head') > -1,
				isBody: chunk.path.indexOf('body') > -1
			}
		...
		.on('data', function (chunk) {
			utils.cacheAsset(chunk.path, chunk.analysis);
			this.resume();
		})
});
