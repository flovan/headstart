...
deps.gulp.task('server', ['browsersync'], function (cb) {
	...
	deps.gulp.watch(['assets/sass/**/*.{scss, sass, css}', '!assets/sass/*ie.{scss, sass, css}'], ['sass-main']);
	deps.gulp.watch(['assets/js/**/view-*.js'], ['scripts-view', 'templates']);
	deps.gulp.watch(['assets/js/**/*.js', '!**/view-*.js'], ['scripts-main', 'templates']);
	deps.gulp.watch(['assets/images/**/*'], ['images']);
	deps.gulp.watch(['templates/**/*'], ['templates']);
...
deps.gulp.task('browsersync', function (cb) {
	...
	deps.browserSync.init(null, {
		server:         deps.lodash.isUndefined(settings.config.proxy) ? {
							baseDir: settings.config.export_templates
						} : false,
