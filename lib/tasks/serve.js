...
deps.taker.task('serve', deps.taker.series('build', function (cb) {
	...
	settings.server.init({
		server:         { baseDir: settings.config.dist.templates },
		ui:             { port: parseInt(settings.config.server.port)+1},
		logConnections: settings.config.server.logConnections,
		logLevel:       settings.config.server.logLevel,
		port:           parseInt(settings.config.server.port),
		proxy:          settings.config.proxy || false,
		tunnel:         settings.isTunnel || null,
		open:           false,
		notify:         false,
		ghostMode: {
			clicks: settings.config.server.sync,
			forms: settings.config.server.sync,
			scroll: settings.config.server.sync
		}
	}, function (err, data) {
		...
		deps.chok.watch([
			...
			if (evt.indexOf('unlink') === 0) {
				...
				deps.ncp(assetPath, trgPath, function (err) {
					...
					if (minifyFlag && (/\.(jpg|jpeg|png|gif|svg)$/i).test(trgPath)) {
						deps.vfs.src(trgPath)
							.pipe(imagemin(settings.config.production.minifyGraphicsOptions))
							.pipe(deps.vfs.dest(trgPath));
					}
