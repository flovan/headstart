...
var questions      = [{
	type:    'confirm',
	message: 'The current directory will be emptied. Continue?',
	name:    'override',
	default: false
}, {
	type:    'confirm',
...
function downloadBoilerplateFiles () {
	console.log(c.grey('Downloading boilerplate files...'));

	// If a custom repo was passed in, validate it
	if (typeof settings.flags.base === 'string') {
		if (settings.flags.base.indexOf('/') < 0) {
			console.log(c.red('Expecting `https://github.com/user/repo#ref`. `new` instruction cancelled.'));
			process.exit(1);
		}

		// Override default repo
		currentRepo = settings.flags.base;
	}

	// Download the boilerplate files to a temp folder
	// This is to prevent a ENOEMPTY error
	deps.ghdownload(currentRepo, settings.tmpFolder)
		...
		.on('end', function () {
			...
			deps.ncp(settings.tmpFolder, settings.cwd, function (err) {
				if (err) {
					console.log(c.red('✘  Unpacking failed somehow. Please try running `new` again...'), err);
					process.exit(1);
				}
...
function finishNew () {
	...
	deps.prompt(finishQuestion, function (answers) {
...
// Define "new" task
deps.taker.task('init', function (cb) {
