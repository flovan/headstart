		...
		.pipe(deps.plugins.tap(function (htmlFile) {
			...
			var
				...
				injectItems =  settings.isProduction ?
					[
						settings.config.export_assets + '/assets/js/core-libs*.min.js',
						settings.config.export_assets + '/assets/js/view-' + viewBaseName + '*.min.js'
					]
					:
					[
						settings.config.export_assets + '/assets/js.s/jquery*.js',
						settings.config.export_assets + '/assets/js.s/ender*.js',

						(settings.isProduction ? '!' : '') + settings.config.export_assets + '/assets/js.s/dev/*.js',

						settings.config.export_assets + '/assets/js.s/*.js',
						settings.config.export_assets + '/assets/js/core/*.js',
						settings.config.export_assets + '/assets/js/**/*.js',

						'!' + settings.config.export_assets + '/assets/**/_*.js',
						'!' + settings.config.export_assets + '/assets/js/ie*.js'
					]
			;

			// Include the css
			injectItems.push(settings.config.export_assets + '/assets/css/main*.css');
			injectItems.push(settings.config.export_assets + '/assets/css/view-' + viewBaseName + '*.css');
			...
			injectItems = deps.gulp.src(injectItems)
				...
				.pipe(deps.plugins.deporder(baseName));
			...
			deps.gulp.src('templates/' + baseName)
				...
				.pipe(deps.plugins.inject(injectItems, {
					ignorePath:   [
							deps.lodash.without(cwdParts, cwdParts.splice(-1)[0]).join('/')
						].concat(settings.config.export_assets.split('/')),
					addRootSlash: false,
