...
deps.gulp.task('templates', deps.gulp.series('clean-rev', function (cb) {
	...
	if (!settings.config.assembleTemplates) {
		deps.gulp.src([settings.templatesFolder + '/**/*', '!' + settings.templatesFolder + '/*.*', '!_*'])
	...
	deps.gulp.src([settings.templatesFolder + '/*.*', '!_*'])
		.pipe(deps.plugins.tap(function (htmlFile) {
			...
			deps.gulp.src(settings.templatesFolder + '/' + baseName)
				.pipe(deps.plugins.plumber())
